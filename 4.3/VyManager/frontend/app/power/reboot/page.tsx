...
export default function PoweroffPage() {
  ...
  const executeReboot = async () => {
    try {
      const response = await fetch(`${apiUrl}/api/reboot`, {
        method: 'POST'
      });
      ...
      if (data.success === true) {
        toast({
            variant: "default",
            title: "Reboot signal received",
            description: "VyOS has received the reboot command",
          });
      } else {
      ...
      toast({
        variant: "destructive",
        title: "Error sending reboot command",
        description:
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-cyan-400">Reboot</h1>
          <p className="text-slate-400">Reboot your VyOS router</p>
        </div>
